...
export async function POST(request: Request) {
  ...
  if (credential.status === 'configuration-missing') {
    ...
    return NextResponse.json({ error: 'Portal authentication is not configured.' }, { status: 500 });
  }
  ...
  try {
    ...
    return NextResponse.json({ error: 'Portal session is unavailable. Please contact an administrator.' }, { status: 500 });
  }
