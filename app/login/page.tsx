...
function safeReturnPath() {
  ...
  return requested.startsWith('/portal') && !requested.startsWith('//') ? requested : '/portal';
}
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 font-sans antialiased">
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center">
        <Link href="/" className="inline-flex items-center gap-3">
          ...
          <div className="text-left">
            <h1 className="text-2xl font-black text-[#0B1E38] tracking-tight">Easy HomeSource</h1>
            <p className="text-xs font-bold text-[#1E6FA8] uppercase tracking-wider">QUOTE PORTAL &amp; OPERATIONS</p>
          </div>
        </Link>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-6 shadow-xl border border-slate-200 sm:rounded-3xl sm:px-10 space-y-6">
          <div>
            <h2 className="text-xl font-black text-[#0B1E38] tracking-tight">Employee sign in</h2>
            <p className="text-xs text-slate-500 font-medium mt-1">Enter your authorized email and password to access the internal quote system.</p>
          </div>

          {error && (
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700 font-semibold leading-relaxed">{error}</div>
          )}
          ...
          <form onSubmit={handleSignIn} className="space-y-4">
            ...
            >
              <span>{loading ? 'Signing in...' : 'Sign in with Email and Password →'}</span>
            </button>
          </form>

          <div className="pt-2 text-center">
            <p className="text-[11px] text-slate-400 font-medium">Authorized dealership personnel only. Public access is restricted.</p>
          </div>
        </div>

        <div className="mt-6 text-center text-xs text-slate-400">
          <Link href="/" className="hover:text-slate-600 font-medium">← Return to Public Website</Link>
        </div>
      </div>
    </div>
  );
