    ...
    <main className="px-4 py-12">
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.9fr_1fr]">
        <section>
          ...
          <h1 className="mt-2 text-4xl font-black text-forest">Prepare for manufactured home financing with clearer next steps.</h1>
          <p className="mt-4 text-lg leading-8 text-forest/75">Easy HomeSource helps Florida buyers get ready for lender conversations without promising approval, rates, payments, or specific loan terms. Our goal is to help you ask better questions and understand the real costs that can affect your move-in plan.</p>
          <div className="mt-8 grid gap-4">
            {financingSections.map(([title, text]) => <div key={title} className="rounded-2xl bg-sand p-5 leading-7 text-forest"><h2 className="text-xl font-black">{title}</h2><p className="mt-2 text-forest/75">{text}</p></div>)}
          </div>
          <p className="mt-6 rounded-2xl border border-forest/10 bg-white p-4 text-sm font-semibold leading-6 text-forest/70">Financing availability, qualification requirements, rates, payments, down payment amounts, and loan terms are determined by lenders and may vary by buyer, property, home, and program.</p>
