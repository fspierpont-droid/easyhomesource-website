...
if (!ONLY) {
  console.error("Conservative public Trove import requires --only=<slug>. Start with --only=paxton before broad importing.");
  process.exit(1);
}
if (!Number.isFinite(DELAY_MS) || DELAY_MS < 8000 || DELAY_MS > 15000) {
  console.error("Conservative public Trove import requires --delay between 8000 and 15000 milliseconds.");
  process.exit(1);
}
...
async function main() {
  ...
  const allowlist = allAllowed.filter((home) => home.slug === ONLY);
  if (!allowlist.length) throw new Error(`No approved home found for --only=${ONLY}`);
  ...
  printSkipped();
  console.log(`Reports written to reports/public-trove-media-review-report.json and reports/public-trove-media-review-report.md`);
}
...
function urlsFromValue(value: string, base: string) {
  if (!value) return [];
  return value.split(",").map((part) => part.trim().split(/\s+/)[0]).map((url) => decodeNextImageUrl(absolute(url, base), base)).filter(Boolean);
}
...
async function writeReports(reports: HomeReport[], phase = "final") { await writeFile(path.join(REPORT_DIR, "public-trove-media-review-report.json"), `${JSON.stringify({ generatedAt: new Date().toISOString(), phase, startUrl: START_URL, only: ONLY, delayMs: DELAY_MS, publicHtmlOnly: true, stopOn429: STOP_ON_429, homes: reports }, null, 2)}\n`); await writeFile(path.join(REPORT_DIR, "public-trove-media-review-report.md"), renderMarkdownReport(reports)); }
...
function decodeNextImageUrl(url: string, base: string) {
  try {
    const parsed = new URL(url, base);
    if (parsed.pathname === "/_next/image" && parsed.searchParams.get("url")) return absolute(parsed.searchParams.get("url") || "", base);
