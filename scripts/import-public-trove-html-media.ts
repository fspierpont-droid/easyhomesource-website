...
const DELAY_MS = Number(argValue("--delay") || 8000);
const LIMIT = Number(argValue("--limit") || 0);
const STOP_ON_429 = true;
...
async function main() {
  const allAllowed = JSON.parse(await readFile(path.join(ROOT, "scripts/trove-media-allowlist.json"), "utf8")) as Allow[];
  let allowlist = ONLY ? allAllowed.filter((home) => home.slug === ONLY) : allAllowed;
  if (LIMIT > 0) allowlist = allowlist.slice(0, LIMIT);
  if (!allowlist.length) throw new Error(ONLY ? `No approved home found for --only=${ONLY}` : "No approved homes found.");
  const manifest: Record<string, unknown> = {};
  const reports: HomeReport[] = [];
  await mkdir(REPORT_DIR, { recursive: true });
  for (const home of allAllowed) {
    for (const category of categories) await mkdir(path.join(ROOT, "public/homes", home.slug, category), { recursive: true });
    manifest[home.slug] = emptyEntry(home.slug, home.troveDetailUrl ?? null);
  }

  for (const [homeIndex, home] of allowlist.entries()) {
    const report = createHomeReport(home);
    reports.push(report);
    try {
      const detailUrl = home.troveDetailUrl || null;
      ...
      if (!detailUrl) {
        ...
        extracted = extractCandidates(html, detailUrl, home);
        Object.assign(report, extracted.counts);
      }
      const allCandidates = extracted.candidates;
      ...
      await writeReports(reports, "review");
      ...
      for (const candidate of limitByCategory(candidates.sort((a, b) => b.score - a.score))) {
    ...
    if (homeIndex < allowlist.length - 1 && DELAY_MS > 0) await delay(DELAY_MS);
  }
  await writeManifest(manifest);
  await writeReports(reports);
  printSkipped();
  console.log(`Reports written to reports/public-trove-html-media-report.json and reports/public-trove-html-media-report.md`);
}
...
function extractCandidates(html: string, pageUrl: string, home: Allow): ExtractedCandidates {
  const out: Candidate[] = [];
  ...
  const add = (url: string, context: string, kind: Candidate["kind"] = "image") => {
    for (const finalUrl of urlsFromValue(url, pageUrl)) {
      ...
      if (belongsToCurrentHome(finalUrl, text, home, pageUrl)) out.push({ url: finalUrl, kind, alt: makeAlt(text, category), category, sourceUrl: pageUrl, score: score(finalUrl, text, category) });
    }
  };
  ...
  for (const m of Array.from(html.matchAll(/<a\b([^>]+)>([\s\S]*?)<\/a>/gi))) {
    ...
    if (/\.(jpe?g|png|webp)(\?|$)/i.test(href) || /\/_next\/image\?[^"']*url=/i.test(href)) add(href, m[2] + m[1], "image");
  ...
  for (const m of Array.from(html.matchAll(/https?:[^"'<>\s]+\.(?:jpe?g|png|webp|pdf)(?:\?[^"'<>\s]*)?|\/_next\/image\?[^"'<>\s]+/gi))) {
    const raw = m[0].replace(/&amp;/g, "&");
    add(raw, "raw html media link", /\.pdf(\?|$)/i.test(raw) ? "brochure" : "image");
  }
  const firstLargeImage = out.find((candidate) => candidate.kind === "image" && candidate.category === "other");
  if (firstLargeImage) { firstLargeImage.category = "exterior"; firstLargeImage.alt = makeAlt(`${home.name} exterior hero image`, "exterior"); }
  return { candidates: out, counts };
}

function urlsFromValue(value: string, base: string) {
  if (!value) return [];
  return value.split(",").map((part) => part.trim().split(/\s+/)[0]).flatMap((url) => nextImageCandidates(absolute(url, base), base)).filter(Boolean);
}
...
function rejectionReason(c: Candidate): string | null { const u = c.url.toLowerCase(); if (c.kind === "video" || c.kind === "brochure") return null; if (/similar|related|recommended|more-homes|other-homes|map|maps|logo|favicon|icon|sprite|badge|buildtrove|pixel|avatar|social|facebook|instagram|youtube|twitter|tiktok|linkedin|pinterest/.test(u)) { skipped.logosIcons++; return "logos/icons/maps/social/similar-home assets"; } if (/(^|[?&])(w|width)=([1-9][0-9]?|1[0-9]{2})\b|(?:-|_)([1-9][0-9]?|1[0-9]{2})x/.test(u)) { skipped.tiny++; return "tiny thumbnails"; } if (!/\.(jpe?g|png|webp|avif)(\?|$)/i.test(u)) return "unsupported image file type"; return null; }
...
async function fetchText(url: string) {
  debug(`Fetching Trove page: ${url}`);
  try {
    const res = await fetch(url, { headers: pageHeaders() });
    if (STOP_ON_429 && res.status === 429) { console.error(`HTTP 429 received from ${url}; stopping immediately.`); process.exit(1); }
...
async function writeReports(reports: HomeReport[], phase = "final") { await writeFile(path.join(REPORT_DIR, "public-trove-html-media-report.json"), `${JSON.stringify({ generatedAt: new Date().toISOString(), phase, startUrl: START_URL, only: ONLY, delayMs: DELAY_MS, publicHtmlOnly: true, stopOn429: STOP_ON_429, homes: reports }, null, 2)}\n`); await writeFile(path.join(REPORT_DIR, "public-trove-html-media-report.md"), renderMarkdownReport(reports)); }
function renderMarkdownReport(reports: HomeReport[]) { const lines = ["# Public Trove Media Review Report", "", `Generated: ${new Date().toISOString()}`, `Only: ${ONLY}`, `Delay: ${DELAY_MS}ms`, `Public HTML only: yes`, `Stop on HTTP 429: yes`, ""]; for (const r of reports) { lines.push(`## ${r.homeName}`, "", `- Slug: ${r.slug}`, `- Matched Trove detail URL: ${r.matchedTroveDetailUrl || "Not found"}`, `- Detail page found: ${r.detailPageFound ? "yes" : "no"}`, `- IMG tags found: ${r.imgTagCount}`, `- Srcset URLs found: ${r.srcsetUrlCount}`, `- PDF/brochure links found: ${r.pdfBrochureLinkCount}`, `- Iframe/video links found: ${r.iframeVideoLinkCount}`, `- Media candidates found: ${r.mediaCandidatesFound}`, `- Media candidates accepted: ${r.mediaCandidatesAccepted}`, `- Media candidates rejected: ${r.mediaCandidatesRejected}`, `- Rejection reasons: ${Object.entries(r.rejectionReasons).map(([k, v]) => `${k} (${v})`).join(", ") || "None"}`, `- Final downloaded files: ${r.finalDownloadedFiles.join(", ") || "None"}`, `- Generated gallery paths: ${r.generatedGalleryPaths.join(", ") || "None"}`, `- Errors: ${r.errors.join("; ") || "None"}`); if (r.note) lines.push(`- Note: ${r.note}`); lines.push(""); } return `${lines.join("\n")}\n`; }
...
function nextImageCandidates(url: string, base: string) {
  try {
    const parsed = new URL(url, base);
    if (parsed.pathname === "/_next/image" && parsed.searchParams.get("url")) {
      const direct = absolute(parsed.searchParams.get("url") || "", base);
      return direct && direct !== parsed.toString() ? [direct, parsed.toString()] : [parsed.toString()];
    }
    return [parsed.toString()];
  } catch { return []; }
}
function pageHeaders() { return { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36", "Accept": "text/html", "Accept-Language": "en-US,en;q=0.9", "Referer": START_URL }; }
function mediaHeaders(referer: string) { return { "User-Agent": pageHeaders()["User-Agent"], "Accept": "image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8", "Accept-Language": "en-US,en;q=0.9", "Referer": referer || START_URL }; }
function belongsToCurrentHome(url: string, text: string, home: Allow, pageUrl: string) {
  const haystack = norm(`${url} ${text}`);
  const aliases = [home.slug, home.troveName, home.name, ...home.aliases].map(norm).filter(Boolean);
  const pageSlug = new URL(pageUrl).pathname.split("/").filter(Boolean).pop() || "";
  if (/similar|related|recommended|logo|icon|favicon|map|social|facebook|instagram|youtube|twitter|tiktok|linkedin|pinterest/.test(haystack)) return false;
  if (aliases.some((alias) => alias && haystack.includes(alias))) return true;
  if (pageSlug && haystack.includes(norm(pageSlug))) return true;
  // Trove detail pages often expose CDN image paths without alt text; accept only when they came from this detail page extraction.
  const pathText = norm(new URL(url).pathname);
  const imageLike = /\.(jpe?g|png|webp|avif)(\?|$)/i.test(url);
  const looksHomeScoped = /\/(homes|models)\//i.test(new URL(url).pathname);
  return imageLike && (!looksHomeScoped || aliases.some((alias) => alias && pathText.includes(alias)));
}
