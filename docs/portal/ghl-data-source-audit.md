...
| `PORTAL_PASSWORD` | Not present | Server-side shared employee portal credential | **Yes** |
...
Production-safety review found the original portal login was browser-local and never compared the submitted password. The server login flow now requires `PORTAL_PASSWORD` to contain the operational employee credential and establishes a signed, HTTP-only, SameSite-strict server session. Missing password configuration returns a configuration error rather than `Invalid credentials`. GHL reads require an active verified EHS employee; writes additionally require the existing Admin or Manager role. Missing sessions return `401`, and authenticated Associates receive `403` on writes. Vercel Preview and Production must configure `PORTAL_PASSWORD` and a unique random `PORTAL_SESSION_SECRET` of at least 32 characters. The Project Map marker HTML also escapes GHL-derived customer label text and does not include a GHL-derived ID in its CSS class.
