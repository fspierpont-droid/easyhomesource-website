...
export const DIRT_PAD_LOAD_PRICING = legacy.DIRT_PAD_LOADS_TABLE;
export const BLOCK_TIE_DOWN_TABLE = legacy.BLOCK_TIE_DOWN_MATRIX;
...
export function calculateDirtPadPricing(loads: number) {
  ...
  return DIRT_PAD_LOAD_PRICING.find((row) => row.loads === count) || DIRT_PAD_LOAD_PRICING[0];
}
...
function makeService(input: MasterServiceInput): ServiceCatalogItem {
...
const serviceInputs: MasterServiceInput[] = [
  ...
  },
  {
    ...
    name: 'Trim Out',
    category: 'mandatory_services',
    description: 'Master Quote 5 trim: single-wide $1,000/$1,100; double-wide $3,000/$3,300; triple-wide $4,000/$4,400 cost/price.',
    defaultCost: 3000,
    defaultPrice: 3300,
    unit: 'job',
    calcType: 'per_side',
  ...
  },
  {
    ...
    category: 'addons',
    description: 'Master Quote 5 selected hookup package price is $1,320. Confirm actual component scope before finalization.',
    defaultCost: 1200,
    defaultPrice: 1320,
    unit: 'job',
  },
...
export const SERVICE_CATALOG: ServiceCatalogItem[] = serviceInputs.map(makeService);
