...
export async function ghlRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const response = await fetch(`${API_BASE}${path}`, { ...init, cache: 'no-store', headers: {
  ...
  if (!response.ok) throw new GhlConnectionError(`GHL request failed (${response.status}): ${await response.text()}`);
...
function nextPageParams(meta: Record<string, unknown> | undefined, currentPage: number) {
  ...
  if (typeof nextPage === 'number' && Number.isInteger(nextPage) && nextPage > currentPage) return { page: String(nextPage) };
  if (typeof nextPage === 'string' && /^\d+$/.test(nextPage) && Number(nextPage) > currentPage) return { page: nextPage };
...
export async function fetchAllOpportunities(pipelineId?: string, fetchPage: PageFetcher = ghlRequest) {
  ...
  const base = { location_id: locationId, limit: '100', ...(pipelineId ? { pipeline_id: pipelineId } : {}) };
  ...
  for (let requestCount = 0; requestCount < 1000; requestCount += 1) {
    ...
    if (!Array.isArray(data.opportunities)) throw new GhlConnectionError('GHL returned malformed opportunity data.');
