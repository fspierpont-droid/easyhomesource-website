  ...
  return (
    <article className="group relative overflow-hidden rounded-[1.75rem] border border-borderGray bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="relative min-h-52 overflow-hidden bg-ehsSoftBlue">
        <HomeImage src={primary?.src} alt={primary?.alt ?? `${home.name} photo`} className="min-h-52 rounded-none" />
        <span className="absolute right-5 top-5"><StatusBadge status={home.status} /></span>
      </div>
      <div className="space-y-5 p-5 sm:p-6">
        <div>{home.modelNumber && <p className="text-sm font-semibold text-ehsBlue">{home.modelNumber}</p>}<h3 className="mt-1 text-2xl font-black text-ehsBlack">{home.name}</h3></div>
        ...
        <div className="rounded-2xl bg-ehsSoftBlue/70 p-4"><p className="text-xs font-black uppercase tracking-wide text-ehsBlack/60">{home.priceLabel ?? "Price"}</p><p className="mt-1 text-3xl font-black text-ehsBlack">{formatHomePrice(home)}</p><p className="mt-3 text-xs font-semibold leading-5 text-ehsBlack/60">{home.priceDisclaimer ?? catalogPriceDisclaimer}</p></div>
        ...
        <div className="relative z-20 flex flex-col gap-3 sm:flex-row">
          <Link className="inline-flex justify-center rounded-full bg-ehsBlue px-5 py-3 text-sm font-black text-white transition hover:bg-ehsMediumBlue" href={`/homes/${home.slug}`}>View Details</Link>
          <Link className="inline-flex justify-center rounded-full border border-ehsBlue bg-white px-5 py-3 text-sm font-black text-ehsBlue transition hover:bg-ehsSoftBlue" href={`/homes/${home.slug}#lead-form`}>Get Pricing</Link>
        </div>
      </div>
    </article>
  );
