javascript:(()=>{ 
  ...
  window.fetch = async function(...args) {
      ...
      clone.text().then(text => {
          try {
              if (text && text.includes('sq') && text.includes('price')) {
  ...
  // Brute force DOM Scrape as a third fallback
  let domScrape = [];

  alert("EHS Mass Capture initialized. Please slowly scroll down the entire /homes page so all homes load. Once you reach the bottom, click the bookmarklet a SECOND time to download the data.");
         ...
         try {
             ...
             if (text.length > 5 && href) {
                 domScrape.push({
                     href: href,
                     text: text.replace(/\n/g, ' | '),
                     image: img ? stripUnsafeQuery(img.src) : null
                 });
             }
         } catch(e) {}
      });
